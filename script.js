...
class Table {
    ...
    static setButtons () {
        ...
        for (let i = 0; i < statusButtons.length; i++) {
            statusButtons[i].addEventListener('click', Operation.toggleBookStatus);
        }

        for (let i = 0; i < deleteButtons.length; i++) {
            deleteButtons[i].addEventListener('click', Operation.removeBook);
        } 
    }
}
...
document.addEventListener("DOMContentLoaded", Table.fill());
newBookButton.addEventListener('click', Operation.validateForm);